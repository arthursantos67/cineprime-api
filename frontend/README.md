...
`listMovies()` also supports partial `title` search and `genre` UUID filtering,
matching the backend query contract. The home catalog reflects discovery
filters in URL parameters (`q`, `status`, and `genre`) and maps `q` to the API
`title` filter after a short debounce.
