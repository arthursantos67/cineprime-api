...
| `GET` | `{{BASE_URL}}/api/v1/catalog/movies/` | No | Optional query: `title=<partial title>`, `genre=<genre_uuid>`, `status=em_cartaz\|pre_venda\|em_breve`, `is_featured=true\|false` |
...
`em_breve` (upcoming).
Movie `title` search is case-insensitive and partial. Movie `genre` filtering
expects an existing genre UUID.
